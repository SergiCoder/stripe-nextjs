...
Public env vars validated once at module load in `src/lib/env.ts` (`NEXT_PUBLIC_API_URL`, `NEXT_PUBLIC_APP_URL`). Optional: `NEXT_PUBLIC_RECAPTCHA_SITE_KEY` — omit or leave empty to keep reCAPTCHA dormant (the proxy disables captcha-related CSP directives and the client hooks return `null`). Validator is zod-free so `src/proxy.ts` can import it without pulling zod into the per-request proxy bundle. Each value is checked with `new URL(value)` at startup, so a non-URL value (e.g. a bare hostname missing a scheme) fails fast at boot rather than later inside a request handler. Import `env` from `@/lib/env` rather than reading `process.env.NEXT_PUBLIC_*` directly — **exception**: `NEXT_PUBLIC_RECAPTCHA_SITE_KEY` must be read via the static `process.env.NEXT_PUBLIC_RECAPTCHA_SITE_KEY` expression in client components so Next.js inlines the value into the browser bundle (dynamic `process.env[name]` reads are not inlined client-side).
...
- **Login/Signup**: server actions call `POST /auth/login/`, `POST /auth/register/`. After registration the user is redirected to `/login?registered=true` — email must be verified before the first login. A failed login with code `email_not_verified` surfaces a resend link via `ResendVerificationLink` (calls `resendVerificationEmail` action → `POST /auth/resend-verification/`; fire-and-forget, always returns success to avoid email enumeration). Email verification itself is `verifyEmail` action → `POST /auth/verify-email/`. Signup and forgot-password forms acquire a reCAPTCHA v3 token via `useRecaptcha()` (`src/lib/recaptcha/useRecaptcha.ts`) before submitting — the token is passed as `captcha_token` to the server action and forwarded to the backend. The resend-verification flow does the same. Login does not use reCAPTCHA to stay frictionless. The `(auth)` route-group layout (`src/app/[locale]/(auth)/layout.tsx`) mounts `RecaptchaScript` once so all auth pages share the same loaded script instance. When `NEXT_PUBLIC_RECAPTCHA_SITE_KEY` is unset, `useRecaptcha()` returns `null` and actions omit the field.
